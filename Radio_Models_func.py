...
class radio_models_fits:
    # This file includes definations of all radio spectral emission models
    ##############################################################################
    ...
    # A screen of ionised gas; external to the gas system can lead to the absorption. 
    ...
    def spec_index_calc(self, x, y, err): 
        ...
        sol = polyfit(X, Y, 1)  
        alpha = sol[0]
        sigma_alpha = np.sqrt((err[0]**2)*(1.0/(np.log10(x[0]/x[1])) * 1.0/(np.log(10)*y[0]))**2 + (err[1]**2)*(1.0/(np.log10(x[0]/x[1])) * -1.0/(np.log(10)*y[1]))**2)
        return alpha, sigma_alpha

    def plot_bowtie(self, nu, alpha_x, flux, e_flux,ax):
        ''' A bowtie plotting function. This is used to show in-band spectral indices
    ...
    def alpha_calc(self, nu, flux, errors):
        ...
        alpha = sol[0]
        #B = sol[1]
        sigma_alpha = np.sqrt((errors[0]**2)*(1.0/(np.log10(nu[0]/nu[1]))*1.0/(np.log(10)*flux[0]))**2 + (errors[1]**2)*(1.0/(np.log10(nu[0]/nu[1]))*-1.0/(np.log(10)*flux[1]))**2)
        return [alpha, sigma_alpha]
    
    
    def par_string(self, fit_par, perr, par_names, funcname):
        if len(fit_par)>2:
            mystring = '\n' + funcname +'\n'+  par_names[0]+ r'= {:.2f} $\pm$ {:.2f}'.format(fit_par[0], perr[0]) +' \n'+par_names[1]+ r'= {:.2f} $\pm$ {:.2f}'.format(fit_par[1], perr[1])+' \n'
            mystring += par_names[2]+ r'= {:.2f} $\pm$ {:.2f} GHz'.format(fit_par[2], perr[2])+'\n'
        else:
            mystring = '\n' + funcname +'\n'+  par_names[0]+ r'= {:.2f} $\pm$ {:.2f}'.format(fit_par[0], perr[0]) +' \n'+par_names[1]+ r'= {:.2f} $\pm$ {:.2f}'.format(fit_par[1], perr[1])+' \n'
    ...
    def tick_offsets(self, f, ax_cord):
        ...
        for i in tick_pos_pix:
            ...
            sky = SkyCoord(ra = dum[0], dec = dum[1], unit = (u.deg, u.deg), frame = 'fk5')
    ...
    def redchisq_calc(self, model, nu, flux, err, pars):
        ...
        red_chi = (1.0/dof)*np.square((model(nu, guess_pars,model_name) -flux)/err)
        return red_chi
    ...
    def generate_model(self, nu_arr, model_name, *pars):
        ...
        n_low = nv*0.99
        n_up = nv*1.01
        flux[~pnu_msk[0]] = np.log10(model_func(n_up, *mypars)/model_func(n_low, 
                        *mypars))/np.log10(n_up/n_low)
        return flux
